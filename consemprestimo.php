...
<div class="mdl-layout mdl-js-layout">
  ...
  include "config.php";
  include "antisql.php";
  $loan = antisql($conexao,$_POST["emprestimo"]);
  $sql = "SELECT * FROM emprestimo WHERE data >= '$loan'";
  $sqlExecuta = mysqli_query($conexao,$sql);
  if(mysqli_num_rows($sqlExecuta) >0){  
    ?>
  <thead>
    <tr>
    <th>Código do emprestimo</th>
      <h1>Livro(s) em locação &nbsp;<i class="material-icons">mood_bad</i></h1>
      <?php while ($linha = mysqli_fetch_assoc($sqlExecuta)){
        ?>
    </tr>
  </thead>
  <tbody>
    ...
    ?>
    <td><i class="material-icons">mood</i>&nbsp;Livros disponível para locação</td>
    <tr>
    <th>Nome</th>
    <th>Editora</th>
    <th>Autor</th>
